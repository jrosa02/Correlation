...
def run_cell(args):
    ...
    result = model.run()
    print(
        f"bp_low={bp_low:.3f}  bp_high={bp_high:.2f}  noise={noise_pow:.3e}  BER={result.ber:.4f}  WER={result.wer:.4f}"
    )
...
if __name__ == "__main__":
    ...
    try:
        ...
        with ProcessPoolExecutor(
            max_workers=min(len(tasks), (os.cpu_count() or 2) // 2), mp_context=mp_ctx
        ) as executor:
