...
def callback(intermediate_result: scipy.optimize.OptimizeResult):
    ...
    print(
        f"iter {len(history):3d} | cost={intermediate_result.fun:.4f} | x={np.round(intermediate_result.x, 4)}"
    )
...
np.save(
    "output/optimize_history.npy", np.array([{"x": h["x"], "fun": h["fun"]} for h in history], dtype=object)
)
