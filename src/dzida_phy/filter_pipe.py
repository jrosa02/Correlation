...
class HighpassModule_Timed(CompoundPipe):
    def __init__(
        ...
        fftplotpipe = (
            FftPlotPipe(fft_plot_input, title=title, sample_rate=sample_rate) if fft_plot_input else None
        )
...
class LowpassModule_Timed(CompoundPipe):
    def __init__(
        ...
        plotpipe = (
            PlotPipe(plot_input, title=f"Lowpass - Cutoff: {lowpass_cutoff}", sample_rate=sample_rate)
            if plot_input
            else None
        )
...
class BandpassModule_Timed(CompoundPipe):
    def __init__(
        ...
        plotpipe = (
            PlotPipe(
                plot_input, title=f"Bandpass - {bandpass_low} / {bandpass_high}", sample_rate=sample_rate
            )
            if plot_input
            else None
        )
