...
class DiodePipe(CompoundPipe):
    def __init__(self, resolution: Quantity, slot_width: Quantity, rise_time: Quantity,
        ...
        self.fftplotpipe = FftPlotPipe(fft_plot_input, title="Diode", sample_rate=resolution, seed=seed) if fft_plot_input else None
        self.slot_width = slot_width
        super().__init__([self.upsampler, lowpass, self.plotpipe, self.fftplotpipe], seed)

class CVLL_350_9(DiodePipe):
    def __init__(self, resolution: Quantity, slot_width: Quantity, plot_input: PlotInput | None = None,
                 fft_plot_input: PlotInput | None = None, seed: int = 42) -> None:
        super().__init__(resolution, slot_width, Quantity((1*ns).to_hz(), display="time"),
                          plot_input, fft_plot_input, seed)
