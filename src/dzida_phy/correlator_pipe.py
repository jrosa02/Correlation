...
class RectCorrModule_Timed(CompoundPipe):
    ...
    def __init__(
        ...
        plotpipe = PlotPipe(plot_input, title='Rect correlator | FPGA',
                            sample_rate=sample_rate) if plot_input else None

        fftplotpipe = None
        if fft_plot_input is not None:
            fftplotpipe = FftPlotPipe(fft_plot_input, title='Rect correlator | FPGA',
                                      sample_rate=sample_rate,
                                      plot_kwargs={'label': 'Correlator output', 'zorder': 1})
            ref = rect_reference(corr.pulse_width)
            plot_fft_spectrum(fftplotpipe.ax, ref, sample_rate,
