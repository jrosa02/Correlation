...
from dzida_phy.signal_pipe import CompoundPipe, SignalPipe, Terminator
...
class DecodeSink_Simple(SignalPipe):
    ...
    def __init__(
        self, data_len: int, chunk_size: int, n_slots: int, sampling_rate: float, seed: int = 42
    ) -> None:
        super().__init__(seed)
...
class DecodePlotSink_Timed(CompoundPipe):
    ...
    def __init__(
        ...
        self._decoder = DecodePipe_Timed(data_len, chunk_size, n_slots, sample_rate, slot_rate, seed)
        plotpipe = (
            PlotPipe(plot_input, "bar", title="Decoded PPM | FPGA", sample_rate=None, seed=seed)
            if plot_input
            else None
        )
